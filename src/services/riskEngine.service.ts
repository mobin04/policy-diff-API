import { Change, RiskedChange, RiskLevel, Section, DiffDetail } from '../types';
...
const SECTION_MULTIPLIER: Record<string, number> = {
  pricing: 1.5,
  refund: 2,
  arbitration: 2,
  liability: 2,
 ...
 */
function normalizeForRisk(text: string): string {
  return text
    .toLowerCase()
 ...
 */
function extractAddedContent(details: DiffDetail[]): string {
  return details
    .filter((d) => d.added)
    .map((d) => d.value)
    .join(' ');
 ...
 */
function extractRemovedContent(details: DiffDetail[]): string {
  return details
    .filter((d) => d.removed)
    .map((d) => d.value)
    .join(' ');
 ...
 */
function detectTransferCluster(text: string): boolean {
    ...
    if (isVerb) {
      ...
      if (!isNegated) {
        ...
        for (let j = i + 1; j <= i + 5 && j < tokens.length; j++) {
          if (SENSITIVE_NOUNS.includes(tokens[j])) return true;
        }
 ...
 */
function detectNegationShift(removedContent: string, addedContent: string): boolean {
  ...
  if (!hasNegationRemoved) return false;
 ...
 */
function scanForRoots(content: string, roots: string[]): boolean {
 ...
 */
function applySectionMultiplier(baseRisk: RiskLevel, sectionTitle: string): RiskLevel {
 ...
 */
function evaluateRiskForSectionV2(change: Change, newSections: Section[], oldSections: Section[]): RiskedChange {
  ...
  if (change.type === 'DELETED') {
    ...
    if (detectTransferCluster(deletedContent)) highTriggerCount++;

    // Trigger 2: High risk roots in deleted content
    if (detectHighRisk(deletedContent)) highTriggerCount++;

    // Trigger 3: High risk title
    if (detectTitleRisk(section) === 'HIGH') highTriggerCount++;
