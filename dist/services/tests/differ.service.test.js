...
describe('Structural Diff Engine Tests', () => {
    ...
    test('should detect TITLE_RENAMED when content is identical but title changed', () => {
        const oldSections = [{ title: 'Privacy Policy', content: 'We value your privacy.', hash: 'hash1' }];
        const newSections = [
            { title: 'Data Protection Policy', content: 'We value your privacy.', hash: 'hash1' },
        ];
        const changes = (0, differ_service_1.diffSections)(oldSections, newSections);
    ...
    test('should detect DELETED and ADDED when title changed and content different (low similarity)', () => {
        const oldSections = [{ title: 'Introduction', content: 'Old content', hash: 'hash-old' }];
        const newSections = [{ title: 'Overview', content: 'New content', hash: 'hash-new' }];
        const changes = (0, differ_service_1.diffSections)(oldSections, newSections);
        // 'Introduction' vs 'Overview' similarity is low
        expect(changes.some((c) => c.type === 'DELETED' && c.section === 'Introduction')).toBe(true);
        expect(changes.some((c) => c.type === 'ADDED' && c.section === 'Overview')).toBe(true);
    });
    ...
    test('should only allow first deterministic match when content is identical across multiple sections', () => {
        ...
        const newSections = [{ title: 'New 1', content: 'Same', hash: 'same-hash' }];
        const changes = (0, differ_service_1.diffSections)(oldSections, newSections);
    ...
    test('should detect MODIFIED when only numbers change (numeric override)', () => {
        const oldSections = [{ title: 'Pricing', content: 'Entry fee is $100.', hash: 'h1' }];
        const newSections = [{ title: 'Pricing', content: 'Entry fee is $100.00.', hash: 'h2' }];
        ...
        const os2 = [{ title: 'Pricing', content: 'Cost: $100', hash: 'h1' }];
        const ns2 = [{ title: 'Pricing', content: 'Cost: $200', hash: 'h2' }];
        const changes = (0, differ_service_1.diffSections)(os2, ns2);
        expect(changes).toHaveLength(1);
        expect(changes[0].type).toBe('MODIFIED');
    });
    ...
    test('should detect no changes when sections are reordered', () => {
        ...
        const oldSections = [s1, s2];
        const newSections = [s2, s1];
        const changes = (0, differ_service_1.diffSections)(oldSections, newSections);
    ...
    test('should ignore trivial changes below 5% threshold', () => {
        ...
        const oldSections = [{ title: 'Long', content: longContent, hash: 'h1' }];
        const newSections = [{ title: 'Long', content: longContent + ' (dot)', hash: 'h2' }];
        const changes = (0, differ_service_1.diffSections)(oldSections, newSections);
    ...
    test('should ignore formatting changes but trigger on value changes', () => {
        ...
        const oldSections = [{ title: 'Fees', content: longContent1, hash: 'h1' }];
        const newSections = [{ title: 'Fees', content: longContent2, hash: 'h2' }];
        const changesA = (0, differ_service_1.diffSections)(oldSections, newSections);
        ...
        const newSectionsB = [{ title: 'Fees', content: longContent3, hash: 'h3' }];
        const changesB = (0, differ_service_1.diffSections)(oldSections, newSectionsB);
        ...
        const osV = [{ title: 'Version', content: longContentV1, hash: 'hv1' }];
        const nsV = [{ title: 'Version', content: longContentV2, hash: 'hv2' }];
        const changesV = (0, differ_service_1.diffSections)(osV, nsV);
        ...
        const osID = [{ title: 'Revision', content: longContentID1, hash: 'hid1' }];
        const nsID = [{ title: 'Revision', content: longContentID2, hash: 'hid2' }];
        const changesID = (0, differ_service_1.diffSections)(osID, nsID);
    ...
    test('should accurately populate matching metrics and handle edge cases', () => {
        ...
        const result = (0, differ_service_1.diffSections)(oldSections, newSections, context);
    ...
    test('should prioritize exact match over rename detection', () => {
        ...
        const result = (0, differ_service_1.diffSections)(os, ns);
        // 'Exact Title' matches Pass 1 (Exact Title)
        // 'New Title' matches Pass 3B (Rename)
        expect(result.filter((c) => c.type === 'MODIFIED')).toHaveLength(1);
        expect(result.filter((c) => c.type === 'TITLE_RENAMED')).toHaveLength(1);
        expect(result.title_rename_count).toBe(1);
    });
});
