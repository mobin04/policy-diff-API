...
describe('Temporal Noise Masking (dateMasker)', () => {
    ...
    describe('Context-Aware Masking', () => {
        test('should mask dates when preceded by known anchors', () => {
            ...
            anchors.forEach((anchor) => {
                ...
                expect((0, dateMasker_1.maskTemporalNoise)(input)).toContain(DATE_TOKEN);
            });
        });
        test('should NOT mask dates without valid anchor keywords', () => {
            const noAnchor = 'Authorization expires on 2026-05-20';
            expect((0, dateMasker_1.maskTemporalNoise)(noAnchor)).not.toContain(DATE_TOKEN);
            expect((0, dateMasker_1.maskTemporalNoise)(noAnchor)).toContain('2026-05-20');
        });
        test('should handle multiple anchored dates in one document', () => {
            const input = 'Effective Date: 2025-01-01. Then nothing happens until 2026-01-01, which is when something happens. Revised on May 10th, 2025.';
            const masked = (0, dateMasker_1.maskTemporalNoise)(input);
    ...
    describe('Hash Stability', () => {
        test('should produce identical hashes for date-only changes', () => {
            ...
            const masked1 = (0, dateMasker_1.maskTemporalNoise)(d1);
            const masked2 = (0, dateMasker_1.maskTemporalNoise)(d2);
            expect(masked1).toBe(masked2);
            expect((0, hash_service_1.generateDateMaskedHash)(masked1)).toBe((0, hash_service_1.generateDateMaskedHash)(masked2));
        });
    });
    describe('Format Support', () => {
        test('should support various date formats near anchors', () => {
            ...
            formats.forEach((format) => {
                ...
                expect((0, dateMasker_1.maskTemporalNoise)(input)).toBe('Updated on __DATE_TOKEN__');
            });
        });
    });
});
